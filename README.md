...
## 2018-11-21
&emsp;&emsp;学习了循环神经网络、语言数据模型。
+ **语言数据模型**
	- 首先读取文件，保存到一个列表中
	- 建立字符索引，一般有 `char_to_idx`、和`idx_to_char`
	- 最重要的时序数据的采样，其中每个样本长度为 `num_steps (时间步长)`,小批量样本形状为_(批量大小，时间步长)_ ，分为随机采样和相邻采样：
		* **随机采样**：代码每次随机从样本中抽出一个小批量,相邻的两个小批量样本之间可能时序上并不连续，所以上一个batch中计算出来的隐藏层状态，不能用于下一个batch的初始化，训练过程中，每个batch开始时，都必须重新初始化隐藏层状态，具体步骤：
			1. 假设一共有的样本数为 `corpous_indices`,所以随机采样能够得到的最大样本数为 `num_examples = (corpous_indices-1) // num_steps` (由于每个样本都有num_step个值)
			2. 对上面生成 `range(num_examples)` 并进行 `shuffle` 操作
			3. 最后通过生成器，每次取出`batch_size`大小个样本，进行训练
		* **相邻采样**：代码每个小批量的训练样本在时间序列上都是连续的，所以上一个小批量计算出来的隐藏层状态可以作为下一次小批量训练的初始化状态。

+ **循环神经网络**
	- gluon中rnn为 `rnn.RNN(num_hiddens)`
		* **输入** (时间步数，批量大小，词量大小)
		* **输出** (时间步数，批量大小，隐藏单元个数)
		* 所以如果直接输入的是（批量大小，时间步数）的文本向量时，需要对其转置后再进行one-hot编码从而可以输入cnn网络。
+ **问题**
	- 计算损失函数需要对标签y进行reshape
	- rnn中容易出现梯度消失和爆炸，所以需要进行梯度裁剪
	- RNN未被充分训练时，发现输出一直在重复，说明RNN对先前状态的记忆能力非常强，启发：需要加入遗忘机制？
	- Gluon实现中，由于隐藏层后面接上全连接输出层时，会将隐藏层输出状态  (时间步数，批量大小，隐藏单元个数) reshape 成为 (时间步数*批量大小，隐藏单元个数) 从而每个batch只需要前向计算一次全连接层，
	相对于手写的RNN，每个batch都要前向计算 num_steps次全连接层，所以Gluon的实现效率很高。
